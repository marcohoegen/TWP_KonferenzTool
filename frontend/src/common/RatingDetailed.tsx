...
export default function RatingDetailed({
  ...
  useEffect(() => {
    if (!averageRating) {
      fetch(apiUrl)
        .then((res) => res.json())
        .then((result: RatingData) => setData(result))
  ...
  }, [apiUrl, averageRating, totalRatings, ratingBreakdown]);
        ...
        stars.push(
          <svg key={i} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="h-6 w-6 text-amber-400">
            <path
              ...
              0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988
              l-4.204 3.602a.563.563 0 00-.182.557l1.285 
              5.385a.562.562 0 01-.84.61l-4.725-2.885
              a.563.563 0 00-.586 0L6.982 20.54a.562.562 
